...
module.exports = {
  ...
  components: {
    ...
    schemas: {
      ...
      RegisterTeacherResponse: {
        type: "object",
        properties: {
          message: { type: "string", example: "Teacher request created and pending admin approval" },
  ...
  paths: {
    "/auth/register": {
      post: {
        tags: ["Auth"],
        summary: "Register a new account (student or teacher). Use multipart/form-data for image + JSON fields.",
        requestBody: {
          required: true,
          content: {
            "multipart/form-data": {
              schema: {
                ...
                properties: {
                  name: { type: "string" },
                  email: { type: "string" },
                  password: { type: "string", description: "Used only if backend handles password (not used in Firebase-only flow). For Firebase-first, create user in Firebase and use /auth/first-login with token." },
                  role: { type: "string", enum: ["student", "teacher"], example: "student" },
                  studentProfile: {
                    type: "string",
                    description: "Stringified JSON, example: {\"learning_goals\":[\"Hifz\"],\"current_level\":\"beginner\"}"
                  },
                  teacherProfile: {
                    type: "string",
                    description: "Stringified JSON, example: {\"bio\":\"...\",\"certificates\":[\"...\"],\"specialties\":[\"...\"]}"
                  },
    ...
    "/teacherRequest": {
      post: {
        tags: ["TeacherRequest"],
        summary: "Create a teacher request (student upgrade OR teacher first-login). Requires Firebase token.",
        security: [{ bearerAuth: [] }],
        requestBody: {
          required: true,
          content: {
      ...
      get: {
        tags: ["TeacherRequest"],
        summary: "List teacher requests (admin only). Supports optional query param ?status=pending|approved|rejected",
        security: [{ bearerAuth: [] }],
        parameters: [
    ...
    "/teacher/request/{id}": {
      patch: {
        tags: ["TeacherRequest"],
        summary: "Admin approve or reject a teacher request. Body: { status: 'approved' | 'rejected' }",
        security: [{ bearerAuth: [] }],
        parameters: [
        ...
        requestBody: {
          ...
          content: {
            "application/json": {
              schema: {
                type: "object",
                properties: { action: { type: "string", enum: ["suspend", "activate"] } },
                required: ["action"]
              }
            }
          }
    ...
    "/firebase/getIdTokenExample": {
      get: {
        tags: ["Auth"],
        summary: "How to get Firebase idToken (example) — not an API; docs only",
        description: "Use Firebase REST API to obtain an idToken for testing. Example cURL:\n\ncurl 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=YOUR_WEB_API_KEY' \\\n -H 'Content-Type: application/json' \\\n -d '{\"email\":\"teacher@gmail.com\",\"password\":\"123456\",\"returnSecureToken\":true}'",
        responses: {
          200: { description: "Use this outside the app to get idToken" }
        }
      }
    }
  }
